...
def main():
    ...
    if existing_files:
        ...
        highest_part = max([int(os.path.basename(f).split('_part_')[1].split('.parquet')[0]) for f in existing_files])
        part_idx = highest_part + 1
        offset = part_idx * CHUNK_SIZE
        print(f"⏩ Found {len(existing_files)} existing files. Resuming from part {part_idx} (offset: {offset:,})...")
    else:
        part_idx = 0
        offset = 0
    # ------------------------
    ...
    while offset < total_rows:
        ...
        df = df.with_columns(
            (pl.col("title") + ": " + pl.col("text").str.slice(0, TEXT_SLICE_LEN)).alias("input_text")
        )
