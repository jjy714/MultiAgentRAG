...
async def DeepResearcherAdvocate(state: DiscussionState) -> dict:
    ...
    try:
        ...
        vote = AdvocateVote(role="deep_researcher", vote="complex", reasoning="Fallback due to parsing error.")

    print(f"[DeepResearcherAdvocate] vote={vote.get('vote')} | reason={vote.get('reasoning')}")
