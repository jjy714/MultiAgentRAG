...
## LangGraph node function that advocates for routing the query to the complex multi-step research tier
def DeepResearcherAdvocate(state: DiscussionState) -> dict:
    """
    args   : {
    ...
    print(f"[DeepResearcherAdvocate] vote={result.get('vote')} | reason={result.get('reasoning')}")
    return {"votes": [result]}
